...
```bash
nix develop            # drops you into a shell with required libraries
cd rust
cargo build --release --features rocm
```

or run it directly via Nix:

```bash
nix run github:kilyanni/ttt-rs#ttt -- <args>
```
...
cargo build --release --features wgpu    # WebGPU, portable but slower
```
