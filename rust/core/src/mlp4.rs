...
/// MLP with 4 hidden layers (5 weight layers total)
/// input(D) -> hidden1(4D) -> hidden2(4D) -> hidden3(4D) -> hidden4(4D) -> output(D)
#[derive(Module, Debug)]
pub struct TTTMLP4<B: GeluBackend> {
    ...
    /// Fourth layer weight: [num_heads, 4*head_dim, 4*head_dim]
    pub w4_init: Param<Tensor<B, 3>>,
    /// Fourth layer bias: [num_heads, 4*head_dim]
    pub b4_init: Param<Tensor<B, 2>>,
    /// Fifth layer weight: [num_heads, 4*head_dim, head_dim]
    pub w5_init: Param<Tensor<B, 3>>,
    /// Fifth layer bias: [num_heads, head_dim]
    pub b5_init: Param<Tensor<B, 2>>,
    pub layer_norm: MultiHeadLayerNorm<B>,
    pub config: Ignored<ModelConfig>,
}

#[derive(Module, Debug)]
pub struct TTTMLP4State<B: GeluBackend> {
    pub w1: Tensor<B, 4>,
    pub b1: Tensor<B, 3>,
    pub w2: Tensor<B, 4>,
    pub b2: Tensor<B, 3>,
...
#[derive(Config, Debug)]
pub struct TTTMLP4Config {
    #[config(default = "Initializer::Normal{mean:0.0, std:0.02}")]
    pub initializer: Initializer,
}

impl Default for TTTMLP4Config {
    fn default() -> Self {
        Self::new()
    }
}

impl<B: GeluBackend> TTTInnerModel<B> for TTTMLP4<B> {
    type Config = TTTMLP4Config;
    type State = TTTMLP4State<B>;
    ...
    fn new(config: &ModelConfig, inner_config: &Arc<Self::Config>, device: &B::Device) -> Self {
        ...
        Self {
            ...
            w5_init: inner_config.initializer.init_with(
                [num_heads, mlp_dim, head_dim],
                Some(len),
                Some(len),
                device,
            ),
            b5_init: inner_config.initializer.init_with(
                [num_heads, head_dim],
                Some(len),
                Some(len),
                device,
    ...
    fn init_state(&self, batch_size: usize) -> Self::State {
        ...
        let w5 = self.w5_init.val().unsqueeze().repeat_dim(0, batch_size);
        let b5 = self.b5_init.val().unsqueeze().repeat_dim(0, batch_size);
    ...
    fn forward_mini_batch(
        ...
        let x5 = gelu_tanh(z4.clone());

        let z5 = x5.clone().matmul(state.w5.clone()) + state.b5.clone().unsqueeze_dim(2);

        let reconstruction_target = qkv.xv - qkv.xk;

        let (_ln_out, grad_l_wrt_z5) = self
            .layer_norm
            .forward_and_l2_grad(z5, reconstruction_target);

        let grad_l_wrt_z4 =
            grad_l_wrt_z5.clone().matmul(state.w5.clone().transpose()) * gelu_bwd(z4);
        ...
        let z4_bar = x4_bar.matmul(state.w4.clone())
            ...
            + b4_bar;
        let x5_bar = gelu_tanh(z4_bar);

        let attn5 = x5_bar.clone().matmul(x5.clone().transpose()).tril(0);
        let b5_bar =
            state.b5.clone().unsqueeze_dim(2) - eta_batch.clone().matmul(grad_l_wrt_z5.clone());
        let z5_bar = x5_bar.matmul(state.w5.clone())
            - (eta_batch.clone() * attn5).matmul(grad_l_wrt_z5.clone())
            + b5_bar;
        ...
        state.b4 = state.b4.clone()
            - (last_eta_col.clone() * grad_l_wrt_z4)
                .sum_dim(2)
                .squeeze_dim::<3>(2);

        state.w5 = state.w5.clone()
            - (last_eta_col.clone() * x5)
                .transpose()
                .matmul(grad_l_wrt_z5.clone());
        state.b5 = state.b5.clone()
            - (last_eta_col * grad_l_wrt_z5)
                .sum_dim(2)
                .squeeze_dim::<3>(2);

        let z5_bar_normalized = self.layer_norm.forward(z5_bar);

        qkv.xq + z5_bar_normalized
    }
}
