...
- **Multiple inner model types**: Linear, Linear+Adam, MLP (2-4 layer variants), naive implementation
- **Mixed layer types**: Mix different inner models across layers (e.g. `"4*linear,mlp"`)
- **Fused GPU kernels**: Hand-written CubeCL kernels that fuse the entire TTT inner loop (forward and backward) into single kernel launches
- **Tiling library**: Custom `thundercube` library providing `St` (shared memory tile) and `Rt` (register tile) tile abstractions with cooperative matrix multiply-accumulate
- **Streaming kernels**: Persistent kernels for pipelined execution (ROCm only)
- **Gradient checkpointing**: Configurable checkpoint intervals
- **Model sizes**: 12M, 60M, 125M, 350M, 760M, 1B parameters
- **Data types**: Supports f32 and bf16
- **Pre-tokenized datasets**: Simple memory-mapped binary format to avoid on-the-fly tokenization
